...
namespace Xekvern\Core\Command\Utils;

use Xekvern\Core\Command\CommandException;
use Xekvern\Core\Command\Utils\Args\BaseArgument;
...
trait ArgumentableTrait {
    ...
    public function registerArgument(int $position, BaseArgument $argument): void {
        if($position < 0) {
            throw new CommandException("You cannot register arguments at negative positions");
        }
        if($position > 0 && !isset($this->argumentList[$position - 1])) {
            throw new CommandException("There were no arguments before $position");
        }
        foreach($this->argumentList[$position - 1] ?? [] as $arg) {
            if($arg instanceof TextArgument) {
                throw new CommandException("No other arguments can be registered after a TextArgument");
            }
            if($arg->isOptional() && !$argument->isOptional()) {
                throw new CommandException("You cannot register a required argument after an optional argument");
            }
        }
    ...
    public function parseArguments(array $rawArgs, CommandSender $sender): array {
        ...
        if(count($rawArgs) > 0) {
            foreach($this->argumentList as $pos => $possibleArguments) {
                ...
                if(!$parsed && !($optional && empty($arg))) { // we tried every other possible argument type, none was satisfied
                    $return["errors"][] = [
                        "code" => Command::ERR_INVALID_ARG_VALUE,
                        "data" => [
        ...
        if($offset < count($rawArgs)) { // this means that the arguments our user sent is more than the needed amount
            $return["errors"][] = [
                "code" => Command::ERR_TOO_MANY_ARGUMENTS,
                "data" => []
            ];
        }
        if($required > 0) {// We still have more unfilled required arguments
            $return["errors"][] = [
                "code" => Command::ERR_INSUFFICIENT_ARGUMENTS,
                "data" => []
            ];
        }
        return $return;
