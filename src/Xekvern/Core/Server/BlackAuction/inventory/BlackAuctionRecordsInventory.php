...
use core\auction\AuctionEntry;
use core\blackauction\BlackAuctionRecord;
use core\blackauction\task\TickRecordsInventory;
use core\Main;
use core\MainPlayer;
...
class BlackAuctionRecordsInventory extends InvMenu {
    ...
    public function __construct(array $entries, int $page = 1) {
        ...
        $this->setListener(self::readonly(function(DeterministicInvMenuTransaction $transaction): void {
            ...
            if($slot === 0) {
                ...
                Main::getInstance()->getScheduler()->scheduleDelayedTask(new ClosureTask(function() use ($player): void {
                    (new BlackAuctionMainInventory())->send($player);
                }), 20);
            }
        ...
        Main::getInstance()->getScheduler()->scheduleRepeatingTask(new TickRecordsInventory($this), 20);
    }
    ...
    public function initItems(): void {
        ...
        foreach($entries as $entry) {
            ...
            $time = Utils::secondsToCD(time() - $entry->getSoldTime());
            $lore = $item->getLore();
            $lore[] = "";
